...
var TOMORROW = (0, import_obsidian.moment)().add(1, "day");
  ...
  display() {
    ...
    for (const periodicity of periodicities) {
      if (settings[periodicity].available) {
        ...
        new import_obsidian4.Setting(this.containerEl).setHeading().setName(`Carry over ${periodicity} tasks`).setDesc(`Whether any ${periodicity} tasks that are incomplete should be automatically carried over to the following note.`).addToggle((toggle) => {
...
var DailyNote = class extends Note {
  getDate() {
    return (0, import_obsidian5.moment)().startOf(UNIT);
  }
  getCurrent() {
...
var WeeklyNote = class extends Note {
  getDate() {
    return (0, import_obsidian6.moment)().startOf(UNIT2);
  }
  getCurrent() {
      ...
      if (settings.tasksAvailable && settings.kanbanSync && periodicitySetting.addDue) {
        ...
        if (board !== void 0) {
          ...
          for (const task of boardTasks.getTasksFromLists([UPCOMING, DUE, PROGRESS])) {
            ...
            if (dueDate && (0, import_obsidian7.moment)(dueDate).isBefore(cls.getNextDate()) && tasksToAdd.find((t) => t.equals(task)) === void 0) {
...
// src/index.ts
var AutoTasks = class extends import_obsidian8.Plugin {
  ...
  onLayoutReady() {
    ...
    this.registerEvent(this.app.vault.on("create", (file) => {
      if (file instanceof import_obsidian8.TFile && file.name !== this.settings.kanbanFile) {
